...
// 스위치 상태 조회용 매핑(제어 코드) — effective 사용(존재 키는 raw 와 동일)
const mapping = computed<Record<string, string | undefined>>(() =>
  props.device ? deviceStore.getEffectiveMapping(props.device) : {}
)
// 표시 채널 — 게이트웨이에서 실제 활성화된 채널(raw channelMapping)만. 기본매핑으로 채워지는
// 비활성 채널(예: 미구성 zone_3/4)은 제외. disabledChannels(onboard/zigbee 토글)도 제외.
const rawMapping = computed<Record<string, string | undefined>>(() =>
  (props.device?.channelMapping || {}) as Record<string, string | undefined>
)
